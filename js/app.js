...
function callback(results, status) {
  if (status == google.maps.places.PlacesServiceStatus.OK) {
    ...
    results.forEach(function (place) {
      place.marker = createMarker(place);
      bounds.extend(new google.maps.LatLng(place.geometry.location.lat(),
        place.geometry.location.lng()));
    });
    map.fitBounds(bounds);
    results.forEach(getAllPlaces);
...
.done(function(response) {
  ...
  var phoneNum = venue.contact.formattedPhone;
  if (phoneNum !== null && phoneNum !== undefined) {
    self.foursquareInfo += 'Phone: ' + phoneNum + '<br>';
  } else {
    self.foursquareInfo += 'Phone: Not Found' + '<br>';
  }
  // Twitter
  var twitterId = venue.contact.twitter;
  if (twitterId !== null && twitterId !== undefined) {
    self.foursquareInfo += 'twitter: @' + twitterId + '<br>';
  } else {
    self.foursquareInfo += 'twitter: Not Found' + '<br>';
  }
  var urlLink = venue.url;
  if (urlLink !== null && urlLink !== undefined){
    self.foursquareInfo += 'Url: ' + urlLink + '<br>';
  } else {
    self.foursquareInfo += 'URL Not Found';
  }
});
};
/*
